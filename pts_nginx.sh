...
extract_results() {
    ...
    for ((i=1; i<=result_count; i++)); do
        local connections=$(xmllint --xpath "string(/PhoronixTestSuite/Result[$i]/Arguments)" $XML_FILE | awk '{print $2}')
        local value=$(xmllint --xpath "string(/PhoronixTestSuite/Result[$i]/Data/Entry/Value)" $XML_FILE)
	local scale=$(xmllint --xpath "string(/PhoronixTestSuite/Result[$i]/Data/Entry/Scale)" $XML_FILE)
        echo "$connections   $value    $scale" >> $OPT_FILE
	echo "" >> $OPT_FILE
    done
    echo "==============================" >> $OPT_FILE
}
